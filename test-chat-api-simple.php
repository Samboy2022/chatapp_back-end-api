...
$user1Token = ''; // Add User 1's token here
$user2Token = ''; // Add User 2's token here
$user3Token = ''; // Add User 3's token here
...
if (empty($user1Token) || empty($user2Token)) {
    echo "❌ Please provide user tokens in the script variables\n";
    echo "   Update user1Token, user2Token, and user3Token variables\n";
    exit(1);
}
...
$user1Info = testStep("Get User 1 Info", function() use ($baseUrl, $userTokens) {
...
$user2Info = testStep("Get User 2 Info", function() use ($baseUrl, $userTokens) {
...
$privateChat = testStep("Create Private Chat between User1 and User2", function() use ($baseUrl, $userTokens, $user2Info) {
    $headers = ['Authorization: Bearer ' . $userTokens['user1']];
...
if (!empty($user3Token)) {
    $groupChat = testStep("Create Group Chat with 3 Users", function() use ($baseUrl, $userTokens, $user2Info) {
        $headers = ['Authorization: Bearer ' . $userTokens['user1']];
...
if (isset($privateChat)) {
    testStep("Pin Private Chat", function() use ($baseUrl, $userTokens, $privateChat) {
        $headers = ['Authorization: Bearer ' . $userTokens['user1']];
    ...
    testStep("Mute Private Chat", function() use ($baseUrl, $userTokens, $privateChat) {
        $headers = ['Authorization: Bearer ' . $userTokens['user1']];
...
if (isset($privateChat)) {
    testStep("Send Reply Message", function() use ($baseUrl, $userTokens, $privateChat) {
        $headers = ['Authorization: Bearer ' . $userTokens['user2']];
    ...
    testStep("Send Location Message", function() use ($baseUrl, $userTokens, $privateChat) {
        $headers = ['Authorization: Bearer ' . $userTokens['user1']];
